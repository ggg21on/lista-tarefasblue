  ...
  <v-app>
    ...
    <v-navigation-drawer app v-model="sidebar" :mini-variant.sync="mini">
      ...
      <v-list-item class="px=2" @click="mini = !mini">
        ...
        <v-list-item-content>Authenticated User</v-list-item-content>
    ...
    <v-footer app class="py-3">
      <span class="overline">Assignment &copy;2021</span>
    </v-footer>
  </v-app>
</template>
...
export default {
  data() {
    return {
      sidebar: true,
      mini: "falseauthenticated user",
      items: [
        { title: "Home", icon: "mdi-home-variant", to: "/" },
        { title: "Profile", icon: "mdi-account-wrench", to: "/profile" },
        { title: "Exit", icon: "mdi-exit-run", to: "/login" },
      ],
    };
  },
